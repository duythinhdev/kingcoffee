...
export class ProductCard implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.isRole == 1 || this.isRole == 2 ||this.isRole == 5){
      this.roleWeFree = true;
    }
    if (this.product) {
      ...
      if(!this.isHome) {
        if(!isEmpty(this.product.promotions)) {
          ...
          if(!isEmpty(productDiscount)) {
            const productDiscountItem = productDiscount[0].productDiscount.filter(x => x.productId === this.product.id)
