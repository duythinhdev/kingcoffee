...
export class SignupComponent implements OnInit {
  ...
  async submit(frm) {
    ...
    if (
      (frm.invalid &&
        this.input.KnowFrom === 1 &&
        this.input.ReferralCode === '') ||
      !this.checkT ||
      !this.checkFormTaxcode ||
      !this.checkValid ||
      (this.input.CompanyName === '' && this.dataInfo.isCompany === 1) ||
      (this.input.TaxCode === '' && this.dataInfo.isCompany === 1)
    ) {
      ...
      await this.checkDuplicateTaxcode();
  ...
  async load_Province() {
    ...
    console.log('this.provinces '+this.provinces.length)
  }
  ...
  async loadWard(districtId) {
    ...
    if (res.data !== undefined) {
      this.wards = res.data;
      console.log('this.wards.length '+this.wards.length)
  ...
  async loadDistrict(provinceId) {
    ...
    if (res.data !== undefined) {
      ...
      console.log('this.districts.length '+this.districts.length)
