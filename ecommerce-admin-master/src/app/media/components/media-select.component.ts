...
export class MediaModalComponent implements OnInit, AfterViewInit {
  ...
  constructor(public activeModal: NgbActiveModal,
     private mediaService: MediaService, private toasty: ToastrService,
  ...
  ngOnInit() {
    ...
    this.fileSelectOptions = Object.assign(this.options, {
      onCompleteItem: (resp) => {this.uploadedFiles.push(resp.data?.photo)},
  ...
  crop() {
    ...
    this.mediaService.upload(this.croppedImage, {
      ...
      .catch(e => this.toasty.error(this.translate.instant(e.data.message || 'Have error, please try again')));
  }
  ...
  update(frm: any) {
    ...
    this.mediaService.update(this.activeEditMedia._id, {
      ...
      .catch(err => this.toasty.error(err.data.message || 'Lỗi hệ thống!'));
  }
  deleteImage(media: any){
    const confirmDelete = confirm(this.translate.instant("Are you sure want to delete this item?"));
    if(confirmDelete){
      this.mediaService.delete(media._id)
        .then(resp => {
...
@Component({
  ...
  template: `<span class="pointer media-select-btn" (click)="open()"><i class="fa fa-folder-open"></i> {{uploadImageTitleModel.Browse}}</span>`
})
