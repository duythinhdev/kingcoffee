...
class Kernel {
  ...
  _modelLoader() {
    ...
    Object.keys(this._modelSchemas).forEach(name => {
      const schema =
        typeof this._modelSchemas[name] === "function"
          ? this._modelSchemas[name]()
          : this._modelSchemas[name];

      if (schema instanceof mongoose.Schema) {
        if (
          this._mongoosePlugins[name] &&
          Array.isArray(this._mongoosePlugins[name])
        ) {
          this._mongoosePlugins[name].forEach(pluginFunctionFactory =>
            schema.plugin(pluginFunctionFactory)
          );
        }
  ...
  jobSchedule() {
    //Job KPI for previous month
    cron.schedule("* * * 1 * *", async () => {
      await Service.KPI.insertMonthKPI(new Date());
      // console.log("Calculate KPI job is running ! ", moment.utc());
    });
  }
  ...
  jobPromotion() {
    //Job KPI for previous month
    cron.schedule("* * * * *", async () => {
      await Service.Notification.insertNotification();
      // console.log("Calculate PROMOTION job is running ! ", moment.locale());
    });
  }
...
function kernelFactory(config) {
  ...
  kernel.loadModule(require("./models/log-events"));
  kernel.loadModule(require("../module/counters/models/counters"));
