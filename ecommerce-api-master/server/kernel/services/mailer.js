...
const mailFrom = 'GoldTimeMart <no-reply@goldtimemart.vn>';
const viewsPath = path.join(__dirname, '..', '..', 'emails');
const sendgridApiKey = nconf.get('SENDGRID_API_KEY');
const sendgridApiKeyNewSendGrid = nconf.get('SENDGRID_API_KEY1');
...
Mailer.prototype.send = async function send(opts) {
 ...
 Mailer.prototype.sendMail = async function sendMail(template, emails, options) {
   console.log(template,emails,options);
  try {
...
let mailer;
let mailerNewSendGrid;
const mailServiceNewSendGrid = nconf.get('MAIL_SERVICE1');
if(mailServiceNewSendGrid === 'sendgrid1')
{
  mailerNewSendGrid = new Mailer(sgTransport({
    auth: {
      api_key: sendgridApiKeyNewSendGrid
    }
  }));
}
...
emailQ.process(async (job, done) => {
  try {
    if(job.data.options.data.type === 'newSendgrid') {
     await mailerNewSendGrid.sendMail(job.data.template, job.data.emails, job.data.options);
    } else {
...
module.exports = {
  send(template, emails, options) {
    console.log("template",template,"email",emails,"option",options)
