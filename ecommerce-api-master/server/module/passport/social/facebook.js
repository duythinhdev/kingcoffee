...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!email) {  email = data.id + `@facebook.com` };
    ...
    if (!user) {
      user = await DB.User.findOne({ providerId: data.id });
    }
    let isNew = false;
    ...
    if (!user) {
      ...
      const loginInvestToken = await Service.SocialConnect.Invest.crossLoginSocial(
        user.username ? user.username : data.id + '_facebook',
        '',
        null,
        validate.value.deviceId
      );
