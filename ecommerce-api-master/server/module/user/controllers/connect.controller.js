  ...
  try {
    const schema = Joi.object().keys({
      accessToken: Joi.string().required().messages({
        'string.base': `"accessToken" bắt buộc phải là những ký tự`,
        'string.empty': `"accessToken" không được để trống`,
        'any.required': `Yêu cầu phải có "accessToken"`
      })
    });
  ...
  try {
    if (!req.session.userId) {
      return next(PopulateResponse.error({
        message: 'Vui lòng đăng nhập trước khi thực hiện yêu cầu này!'
      }));
    }
