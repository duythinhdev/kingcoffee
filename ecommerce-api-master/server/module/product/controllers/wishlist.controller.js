  ...
  try {
    ...
    if (wishlist) {
      return next(PopulateResponse.error({
        message: 'Sản Phẩm đã được thêm vào danh sách ưa thích'
      }));
      // res.locals.wishlist = wishlist;
    }
...
exports.remove = async (req, res, next) => {
  try {
    if (!req.user.userRoles.find(x => x.Role === Enums.UserEnums.UserRole.Admin.value) && req.user._id.toString() !== req.wishlist.userId.toString()) {
      return next(PopulateResponse.forbidden());
    }

    await req.wishlist.remove();
    res.locals.remove = {
      message: 'Đã Xóa Danh Sách Ưa Thích'
    };
    return next();
  } catch (e) {
    return next(e);
...
exports.list = async (req, res, next) => {
  ...
  try {
    ...
    const count = await DB.Wishlist.countDocuments(query);
