...
const validateSchema = Joi.object().keys({
  type: Joi.string().allow('private').optional().default('private').messages({
    'string.base': `"type" bắt buộc phải là những ký tự`,
    'string.empty': `"type" không được để trống`
  }),
  recipientId: Joi.string().allow(null, '').optional().messages({
    'string.base': `"recipientId" bắt buộc phải là những ký tự`,
  }),
});
  ...
  try {
    ...
    if (req.user._id.toString() === validate.value.recipientId) {
      return next(PopulateResponse.error({
        message: 'Không được tạo cuộc trò chuyện với chính bản thân!'
      }));
    }
...
exports.mute = async (req, res, next) => {
  try {
    ...
    res.locals.mute = { succes: true, message: "Đã tắt cuộc trò chuyện" };
    next();
  } catch (e) {
    next(e);
  }
...
exports.unmute = async (req, res, next) => {
  try {
    ...
    res.locals.unmute = { succes: true, message: "Đã bật cuộc trò chuyện" };
    next();
  } catch (e) {
    next(e);
  }
